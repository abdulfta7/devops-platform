...
router.get('/:id', authenticate, async (req, res) => {
  ...
  const enrollment = await db.prepare('SELECT id, is_approved FROM enrollments WHERE user_id = ? AND course_id = ?').get(req.user.id, task.cid);
  ...
  const submission = await db.prepare('SELECT * FROM task_submissions WHERE task_id = ? AND user_id = ? ORDER BY submitted_at DESC LIMIT 1').get(task.id, req.user.id);
...
router.post('/:id/submit', authenticate, async (req, res) => {
  ...
  const task = await db.prepare('SELECT t.*, c.is_free, c.id as cid FROM tasks t JOIN courses c ON c.id = t.course_id WHERE t.id = ?').get(req.params.id);
  if (!task) return res.status(404).json({ error: 'Task not found' });
  ...
  const id = uuidv4();
  await db.prepare('INSERT INTO task_submissions (id, task_id, user_id, answer, status) VALUES (?, ?, ?, ?, ?)').run(id, task.id, req.user.id, answer, 'pending');
