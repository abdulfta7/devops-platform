...
router.post('/enroll', authenticate, (req, res) => {
  ...
  db.prepare('INSERT INTO enrollments (id, user_id, course_id, payment_status, is_approved) VALUES (?, ?, ?, ?, ?)').run(uuidv4(), req.user.id, course_id, 'approved', 0);
...
// Admin: Get stats
router.get('/admin/stats', authenticate, isAdmin, (req, res) => {
  try {
    const stats = {
      total_users: db.prepare("SELECT COUNT(*) as c FROM users WHERE role = 'student'").get().c,
      total_enrollments: db.prepare("SELECT COUNT(*) as c FROM enrollments").get().c,
      pending_tasks: db.prepare("SELECT COUNT(*) as c FROM task_submissions WHERE status = 'pending'").get().c,
      total_courses: db.prepare("SELECT COUNT(*) as c FROM courses WHERE is_published = 1").get().c,
      pending_approvals: db.prepare("SELECT COUNT(*) as c FROM users WHERE is_approved = 0").get().c,
      pending_enrollments: db.prepare("SELECT COUNT(*) as c FROM enrollments WHERE is_approved = 0").get().c,
    };
    res.json(stats);
  } catch (err) {
...
router.get('/admin/pending-enrollments', authenticate, isAdmin, (req, res) => {
  const enrollments = db.prepare(`
...
router.put('/admin/enrollments/:id/approve', authenticate, isAdmin, (req, res) => {
  const enrollment = db.prepare('SELECT * FROM enrollments WHERE id = ?').get(req.params.id);
  if (!enrollment) return res.status(404).json({ error: 'Enrollment not found' });

  db.prepare('UPDATE enrollments SET is_approved = 1, approved_by = ?, approved_at = CURRENT_TIMESTAMP WHERE id = ?').run(req.user.id, req.params.id);
  res.json({ success: true, message: 'Enrollment approved successfully!' });
});

// Admin: Reject enrollment
router.put('/admin/enrollments/:id/reject', authenticate, isAdmin, (req, res) => {
  const enrollment = db.prepare('SELECT * FROM enrollments WHERE id = ?').get(req.params.id);
  if (!enrollment) return res.status(404).json({ error: 'Enrollment not found' });
