...
router.get('/:slug', (req, res) => {
  ...
  const videos = db.prepare('SELECT * FROM videos WHERE course_id = ? ORDER BY order_num').all(course.id);
  const tasks = db.prepare('SELECT id, title, description, difficulty, order_num FROM tasks WHERE course_id = ? ORDER BY order_num').all(course.id);
...
// Check enrollment
router.get('/:slug/enrollment', authenticate, (req, res) => {
    ...
    try {
      ...
      if (user) {
        ...
        if (hasAccess) {
          ...
          const progress = db.prepare('SELECT video_id, watched, progress_seconds FROM video_progress WHERE user_id = ?').all(user.id);
          const progressMap = {};
...
router.post('/:slug/videos/:videoId/progress', authenticate, (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM video_progress WHERE user_id = ? AND video_id = ?').get(req.user.id, video.id);
  if (existing) {
    db.prepare('UPDATE video_progress SET watched = ?, progress_seconds = ? WHERE user_id = ? AND video_id = ?').run(watched ? 1 : 0, progress_seconds || 0, req.user.id, video.id);
  } else {
    db.prepare('INSERT INTO video_progress (id, user_id, video_id, watched, progress_seconds) VALUES (?, ?, ?, ?, ?)').run(uuidv4(), req.user.id, video.id, watched ? 1 : 0, progress_seconds || 0);
  }
  res.json({ success: true });
});

// Admin: Create course
router.post('/', authenticate, isAdmin, upload.single('thumbnail'), (req, res) => {
  const { track_id, title, slug, description, price, is_free, level, duration_hours, order_num } = req.body;
  const id = uuidv4();
  const thumbnail = req.file ? req.file.path : null;
  db.prepare('INSERT INTO courses (id, track_id, title, slug, description, thumbnail, price, is_free, level, duration_hours, order_num) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, track_id, title, slug, description, thumbnail, price || 0, is_free === 'true' || is_free === true ? 1 : 0, level || 'beginner', parseInt(duration_hours) || 0, parseInt(order_num) || 0);
...
router.put('/:id', authenticate, isAdmin, (req, res) => {
  ...
  db.prepare('UPDATE courses SET title=?, description=?, price=?, is_free=?, level=?, duration_hours=?, order_num=?, is_published=? WHERE id=?').run(title, description, price, is_free ? 1 : 0, level, duration_hours, order_num, is_published ? 1 : 0, req.params.id);
  res.json({ success: true });
});
...
router.post('/:courseId/videos', authenticate, isAdmin, upload.single('video'), (req, res) => {
  ...
  db.prepare('INSERT INTO videos (id, course_id, title, description, video_url, duration_minutes, order_num, is_preview) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, req.params.courseId, title, description, finalUrl, duration_minutes || 0, order_num || 0, is_preview ? 1 : 0);
  res.json({ id, title });
});

// Admin: Add task to course
router.post('/:courseId/tasks', authenticate, isAdmin, (req, res) => {
  const { title, description, instructions, expected_output, difficulty, order_num } = req.body;
  const id = uuidv4();
  db.prepare('INSERT INTO tasks (id, course_id, title, description, instructions, expected_output, difficulty, order_num) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, req.params.courseId, title, description, instructions, expected_output, difficulty || 'medium', order_num || 0);
  res.json({ id, title });
});

// Admin: Delete video
router.delete('/videos/:videoId', authenticate, isAdmin, (req, res) => {
  db.prepare('DELETE FROM videos WHERE id = ?').run(req.params.videoId);
  res.json({ success: true });
});

// Admin: Update video
router.put('/videos/:videoId', authenticate, isAdmin, (req, res) => {
  const { title, description, video_url, duration_minutes, order_num, is_preview } = req.body;
  db.prepare('UPDATE videos SET title=?, description=?, video_url=?, duration_minutes=?, order_num=?, is_preview=? WHERE id=?')
...
router.get('/:courseId/manage', authenticate, isAdmin, (req, res) => {
  const course = db.prepare('SELECT c.*, t.title as track_title FROM courses c LEFT JOIN tracks t ON t.id = c.track_id WHERE c.id = ?').get(req.params.courseId);
  if (!course) return res.status(404).json({ error: 'Course not found' });
  const videos = db.prepare('SELECT * FROM videos WHERE course_id = ? ORDER BY order_num').all(course.id);
  const tasks = db.prepare('SELECT * FROM tasks  WHERE course_id = ? ORDER BY order_num').all(course.id);
  res.json({ ...course, videos, tasks });
});

// Admin: Delete task
router.delete('/tasks/:taskId', authenticate, isAdmin, (req, res) => {
...
// Admin: Update task
router.put('/tasks/:taskId', authenticate, isAdmin, (req, res) => {
  const { title, description, instructions, expected_output, difficulty, order_num } = req.body;
  db.prepare('UPDATE tasks SET title=?, description=?, instructions=?, expected_output=?, difficulty=?, order_num=? WHERE id=?')
...
router.post('/:courseId/videos/upload', authenticate, isAdmin, upload.single('file'), (req, res) => {
  const { title, description, video_url, duration_minutes, order_num, is_preview } = req.body;
  const id = uuidv4();
  const finalUrl = req.file ? req.file.path : video_url;
  if (!finalUrl) return res.status(400).json({ error: 'Provide a file or video_url' });
  db.prepare('INSERT INTO videos (id, course_id, title, description, video_url, duration_minutes, order_num, is_preview) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
