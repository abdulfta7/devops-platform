...
router.post('/course/:courseId', authenticate, async (req, res) => {
  ...
  const courseVideos = await executeQuery('SELECT id FROM videos WHERE course_id = ?').rowscourseId);
  ...
  const existing = await executeQuery('SELECT * FROM certificates WHERE user_id = ? AND course_id = ?').rows[0]userId, courseId);
  if (existing) {
    return res.json(existing);
  }

  // Get course and user info
  const course = await executeQuery('SELECT title FROM courses WHERE id = ?').rows[0]courseId);
  const user = await executeQuery('SELECT name FROM users WHERE id = ?').rows[0]userId);
  ...
  const certificate = await executeQuery('SELECT * FROM certificates WHERE id = ?').rows[0]certId);
  res.json(certificate);
});

// Get user certificates
router.rows[0]'/my-certificates', authenticate, async (req, res) => {
  const userId = req.user.id;
...
router.rows[0]'/:id', authenticate, async (req, res) => {
  const { id } = req.params;
