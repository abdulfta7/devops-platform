...
router.post('/register', validateRegistration, (req, res) => {
  ...
  db.prepare('INSERT INTO users (id, name, email, password, phone, verification_token) VALUES (?, ?, ?, ?, ?, ?)')
    .run(id, name, email, hash, phone, verificationToken);

  const token = jwt.sign({ id }, process.env.JWT_SECRET, { expiresIn: '7d' });
  const user = db.prepare('SELECT id, name, email, phone, role, avatar, is_approved, is_verified FROM users WHERE id = ?').get(id);
...
// Update Avatar
router.put('/profile/avatar', authenticate, upload.single('avatar'), (req, res) => {
  if (!req.file) {
    return res.status(400).json({ error: 'No image uploaded' });
  }
  db.prepare('UPDATE users SET avatar = ? WHERE id = ?').run(req.file.path, req.user.id);
  const user = db.prepare('SELECT id, name, email, phone, role, avatar FROM users WHERE id = ?').get(req.user.id);
  res.json({ success: true, user });
});
